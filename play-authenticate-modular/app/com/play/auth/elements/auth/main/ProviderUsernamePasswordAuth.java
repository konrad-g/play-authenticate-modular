...
import com.play.auth.elements.auth.main.EntryTokenAction.Type;
...
public class ProviderUsernamePasswordAuth
	...
	protected Call userExists(final UsernamePasswordAuthUser authUser) {
		return com.play.auth.controllers.routes.AuthController.exists();
	}

	@Override
	protected Call userUnverified(final UsernamePasswordAuthUser authUser) {
		return com.play.auth.controllers.routes.AuthController.unverified();
	}
	...
	protected Body getVerifyEmailMailingBody(final String token,
		...
		final String url = com.play.auth.controllers.routes.AuthController.verify(token).absoluteURL(
	...
	protected Body getPasswordResetMailingBody(final String token,
		...
		final String url = com.play.auth.controllers.routes.AuthController.resetPassword(token).absoluteURL(
				ctx.request(), isSecure);
